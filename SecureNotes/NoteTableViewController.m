...
- (void)insertNewObject:(id)sender
{
    ...
    Lock *temp = [[Lock alloc] init];
    newNote.user = [temp returnUserLogin:self.username];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"showDetail"]) {
        ...
        Notes *object = [[self fetchedResultsController] objectAtIndexPath:indexPath];
        [[segue destinationViewController] setNoteDetail:object];
        [[segue destinationViewController] setContext:context];
        [[segue destinationViewController] setEntity:entity];
    }
    if ([[segue identifier] isEqualToString:@"newNote"]) {
        NSManagedObjectContext *context = [self.fetchedResultsController managedObjectContext];
        NSEntityDescription *entity = [[self.fetchedResultsController fetchRequest] entity];
        Notes *object = [[self fetchedResultsController] objectAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
        [[segue destinationViewController] setNoteDetail:object];
        [[segue destinationViewController] setContext:context];
        [[segue destinationViewController] setEntity:entity];
    }
...
- (NSFetchedResultsController *)fetchedResultsController
{
    ...
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"user.username == %@",self.username];
